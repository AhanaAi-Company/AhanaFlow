...
class HNSWBuilder:
    ...
    def _distance_batch(
        ...
        """Vectorized distance computation for a batch of targets."""
        if not target_indices:
            return np.array([], dtype=np.float32)
        idx_arr = np.asarray(target_indices, dtype=np.int64)
        sub_matrix = matrix[idx_arr]
        sub_norms = norms[idx_arr]
        return self._dist_fn(query, sub_matrix, sub_norms)
        ...
        for lc in range(start_layer, stop_layer_exclusive, -1):
            ...
            while changed:
                ...
                if not neighbor_indices:
                    break
                distances = self._distance_batch(query, neighbor_indices, matrix, norms)
                if distances.size == 0:
                    break
    ...
    def insert(
        ...
        ep = idx.entry_point
        ep_dist = self._distance(query, ep, matrix, norms)

        # Phase 1: Greedy traverse from top to level+1 (find single nearest at each layer)
        ...
        for lc in range(min(level, idx.max_level), -1, -1):
            ...
            candidates = self._search_layer(query, ep, config.ef_construction, lc, matrix, norms)
            ...
            for n_idx, _ in neighbors:
                ...
                if len(n_node.neighbors[lc]) > M_max_n:
                    n_vec = matrix[n_idx]
                    valid_neighbors = [ni for ni in n_node.neighbors[lc]
                                       if ni in idx.nodes and not idx.nodes[ni].deleted]
                    if valid_neighbors:
    ...
    def _search_layer(
        ...
        ep_dist = self._distance(query, entry_point, matrix, norms)
    ...
    def search(
        ...
        ef = max(ef, top_k)  # ef must be >= top_k
        ...
        candidates = self._search_layer(query, ep, ef, 0, matrix, norms)

        # Filter to active-only and return top_k
        results: list[tuple[int, float]] = []
        for c_idx, c_dist in candidates:
            if bool(active[c_idx]):
                results.append((c_idx, c_dist))
            if len(results) >= top_k * 2:  # Gather extra for safety
                break
    ...
    def build_from_matrix(
        ...
        # Shuffle insertion order for better graph quality
        order = active_indices.copy()
        rng = np.random.default_rng(self._config.seed)
        rng.shuffle(order)

        for node_index in order:
            self.insert(int(node_index), matrix, norms)
...
class HNSWLibBackend:
    ...
    def build_from_matrix(
        ...
        lib_idx.set_ef(max(self._config.ef_search, 1))
        ...
        # PQ: train and attach to meta (same logic as HNSWBuilder)
    ...
    def search(
        ...
        # Request extra candidates so active filter has room to trim
    ...
    def mark_deleted(self, node_index: int) -> None:
        ...
        if self._lib_index is not None:
            try:
                ...
                pass  # already deleted or label not in index
    ...
    def upsert_vector(self, node_index: int, vector: np.ndarray) -> bool:
        ...
        if self._lib_index is None:
            # Index not yet built — caller must do full build_from_matrix first
            return False

        vec = vector.astype(np.float32).reshape(1, -1)
        label = np.array([node_index], dtype=np.int64)

        # If the label already exists, mark it deleted first (hnswlib requirement).
        try:
            self._lib_index.mark_deleted(node_index)
        except Exception:
            pass  # label not present — that's fine

        # Resize if needed to accommodate the new element.
